...
mod tests {
    ...
    fn test_normalize_set_name() {
        assert_eq!(normalize_set_name("ads"), "ads");
        assert_eq!(normalize_set_name("my-ads"), "my-ads");
        assert_eq!(normalize_set_name("my_ads"), "my_ads");
        assert_eq!(normalize_set_name("my.ads"), "my_ads");
        assert_eq!(normalize_set_name("my ads"), "my_ads");
        assert_eq!(normalize_set_name("123"), "123");
        assert_eq!(normalize_set_name("1-2-3"), "1-2-3");
        assert_eq!(normalize_set_name("test.com"), "test_com");
    }

    #[test]
    fn test_normalize_set_name_empty() {
        assert_eq!(normalize_set_name(""), "domain-set");
        // non-alphanumeric chars are replaced with underscores
        assert_eq!(normalize_set_name("..."), "___");
        assert_eq!(normalize_set_name("---"), "---");
    }

    #[test]
    fn test_make_cache_file_name() {
        let name1 = make_cache_file_name("ads", "https://example.com/ads.txt");
        let name2 = make_cache_file_name("ads", "https://example.com/ads.txt");
        assert_eq!(name1, name2);

        let name3 = make_cache_file_name("ads", "https://example.com/ads2.txt");
        assert_ne!(name1, name3);

        assert!(name1.contains(".list"));
    }

    #[test]
    fn test_read_to_domain_set() {
        let mut domain_set = HashSet::new();
        let input = "# comment\nads.example.com\n*.test.com\n";
        read_to_domain_set(input, &mut domain_set);

        assert_eq!(domain_set.len(), 2);
        assert!(domain_set.contains(&WildcardName::from_str("ads.example.com").unwrap()));
        assert!(domain_set.contains(&WildcardName::from_str("*.test.com").unwrap()));
    }

    #[test]
    fn test_read_to_domain_set_empty() {
        let mut domain_set = HashSet::new();
        let input = "# only comments\n\n";
        read_to_domain_set(input, &mut domain_set);

        assert_eq!(domain_set.len(), 0);
    }
    ...
    fn test_cache_file_path() {
        let provider = DomainSetHttpProvider {
            name: "test".to_string(),
            url: Url::parse("https://example.com/test.txt").unwrap(),
            interval: Some(60),
            content_type: Default::default(),
        };

        let cache_dir = PathBuf::from("/tmp/cache");
        let cache_file = provider.cache_file_path("test", &cache_dir);

        assert_eq!(cache_file.parent().unwrap(), Path::new("/tmp/cache"));
        assert!(cache_file.file_name().unwrap().to_str().unwrap().ends_with(".list"));
    }
}
