...
| bind-https | DNS Over Https 监听地址 | :sparkles: | [::]:443 | 可绑定多个端口。<br/>IP:PORT: 服务器 IP:端口号<br/>-server_name 服务名<br />-ssl-certificate 证书<br />-ssl-certificate-key 证书私钥<br />[-group]: 请求时使用的 DNS 服务器组<br/>[-no-rule-addr]：跳过 address 规则<br/>[-no-rule-nameserver]：跳过 nameserver 规则<br/>[-no-rule-ipset]：跳过 ipset 和 nftset 规则。<br/>[-no-rule-soa]：跳过 SOA(#) 规则<br/>[-no-dualstack-selection]：停用双栈测速<br/>[-no-speed-check]：停用测速<br/>[-no-cache]：停止缓存 | `bind-https ::443 -server-name dns.exmaple.com -ssl-certificate /etc/certs/dns.exmaple.com.crt -ssl-certificate-key /etc/certs/dns.exmaple.com.key` |
| bind-tls | DNS Over Tls 监听地址 | :sparkles: | [::]:853 | 可绑定多个端口。<br/>IP:PORT: 服务器 IP:端口号<br/>-server_name 服务名<br />-ssl-certificate 证书<br />-ssl-certificate-key 证书私钥<br />[-group]: 请求时使用的 DNS 服务器组<br/>[-no-rule-addr]：跳过 address 规则<br/>[-no-rule-nameserver]：跳过 nameserver 规则<br/>[-no-rule-ipset]：跳过 ipset 和 nftset 规则。<br/>[-no-rule-soa]：跳过 SOA(#) 规则<br/>[-no-dualstack-selection]：停用双栈测速<br/>[-no-speed-check]：停用测速<br/>[-no-cache]：停止缓存<br /> | `bind-tls ::853 -server-name dns.exmaple.com -ssl-certificate /etc/certs/dns.exmaple.com.crt -ssl-certificate-key /etc/certs/dns.exmaple.com.key` |
| cache-size                       | 域名结果缓存个数                           | :white_check_mark: | 512                                                          | 大于等于 0 的数字                                            | cache-size 512                                               |
| cache-persist                    | 是否持久化缓存                             | :white_check_mark: | 自动。<br>当 cache-file 所在的位置有超过 128 MB 的可用空间时启用，否则禁用。 | [yes\|no]                                                    | cache-persist yes                                            |
| cache-file                       | 缓存持久化文件路径                         | :white_check_mark: | /tmp/smartdns.cache                                          | 合法路径字符串                                               | cache-file /tmp/smartdns.cache                               |
| tcp-idle-time                    | TCP 链接空闲超时时间                       | :white_check_mark:     | 120                                                          | 大于等于 0 的数字                                            | tcp-idle-time 120                                            |
...
| rr-ttl-reply-max                 | 允许返回给客户端的最大 TTL 值              | :white_check_mark: | 远程查询结果                                                 | 大于 0 的数字                                                | rr-ttl-reply-max 60                                          |
| local-ttl                        | 本地HOST，address的TTL值                   | :white_check_mark: | rr-ttl-min                                                   | 大于 0 的数字                                                | local-ttl  60                                                |
| max-reply-ip-num                 | 允许返回给客户的最大IP数量                 | :white_check_mark: | IP数量                                                       | 大于 0 的数字                                                | max-reply-ip-num 1                                           |
...
| server                           | 上游 UDP DNS                               | :white_check_mark: | 无                                                           | 可重复。<br>[ip][:port]：服务器 IP:端口（可选）<br>[-blacklist-ip]：配置 IP 过滤结果。<br>[-whitelist-ip]：指定仅接受参数中配置的 IP 范围<br>[-group [group] ...]：DNS 服务器所属组，比如 office 和 foreign，和 nameserver 配套使用<br>[-exclude-default-group]：将 DNS 服务器从默认组中排除<br />[-set-mark]：设置数据包标记so-mark。<br/>[-proxy name]：设置代理服务器。 | server 8.8.8.8:53 -blacklist-ip -group g1                    |
| server-tcp                       | 上游 TCP DNS                               | :white_check_mark: | 无                                                           | 可重复。<br>[ip][:port]：服务器 IP:端口（可选）<br>[-blacklist-ip]：配置 IP 过滤结果<br>[-whitelist-ip]：指定仅接受参数中配置的 IP 范围。<br>[-group [group] ...]：DNS 服务器所属组，比如 office 和 foreign，和 nameserver 配套使用<br>[-exclude-default-group]：将 DNS 服务器从默认组中排除<br />[-set-mark]：设置数据包标记so-mark。<br/>[-proxy name]：设置代理服务器。 | server-tcp 8.8.8.8:53                                        |
...
| proxy-server | 代理服务器 | :white_check_mark: | 无 | 可重复。<br />proxy-server URL <br />[URL]:  [socks5\|http]://[username:password@]host:port<br />[-name]: 代理服务器名称。 | proxy-server socks5://user:pass@1.2.3.4:1080 -name proxy |
| speed-check-mode                 | 测速模式选择                               | :white_check_mark: | 无                                                           | [ping\|tcp:[80]\|http[:80]\|https[:443]\|none]            | speed-check-mode ping,tcp:80,https                   |
| response-mode                    | 首次查询响应模式                           | :white_check_mark: | first-ping                                                   | 模式：[fisrt-ping\|fastest-ip\|fastest-response]<br> [first-ping]: 最快ping响应地址模式，DNS上游最快查询时延+ping时延最短，查询等待与链接体验最佳;<br>[fastest-ip]: 最快IP地址模式，查询到的所有IP地址中ping最短的IP。需等待IP测速; <br>[fastest-response]: 最快响应的DNS结果，DNS查询等待时间最短，返回的IP地址可能不是最快。 | response-mode first-ping                                     |
...
| bogus-nxdomain                   | 假冒 IP 地址过滤                           | :white_check_mark: | 无                                                           | [ip/subnet]，可重复                                          | bogus-nxdomain 1.2.3.4/16                                    |
| ignore-ip                        | 忽略 IP 地址                               | :white_check_mark: | 无                                                           | [ip/subnet]，可重复                                          | ignore-ip 1.2.3.4/16                                         |
...
| force-AAAA-SOA                   | 强制 AAAA 地址返回 SOA                     | :white_check_mark:     | no                                                           | [yes\|no]                                                    | force-AAAA-SOA yes                                           |
| force-qtype-SOA                  | 强制指定 qtype 返回 SOA                    | :white_check_mark:     | qtype id                                                     | [<qtypeid> \| ...]                                           | force-qtype-SOA 65 28                                        |
| prefetch-domain                  | 域名预先获取功能                           | :white_check_mark: | no                                                           | [yes\|no]                                                    | prefetch-domain yes                                          |
| dnsmasq-lease-file               | 支持读取dnsmasq dhcp文件解析本地主机名功能 | :white_check_mark:     | 无                                                           | dnsmasq dhcp lease文件路径                                   | dnsmasq-lease-file /var/lib/misc/dnsmasq.leases              |
| serve-expired                    | 过期缓存服务功能                           | :white_check_mark: | yes                                                          | [yes\|no]，开启此功能后，如果有请求时尝试回应 TTL 为 0 的过期记录，并发查询记录，以避免查询等待 |                                                              |
| serve-expired-ttl                | 过期缓存服务最长超时时间                   | :white_check_mark: | 0                                                            | 秒，0 表示停用超时，大于 0 表示指定的超时的秒数              | serve-expired-ttl 0                                          |
| serve-expired-reply-ttl          | 回应的过期缓存 TTL                         | :white_check_mark: | 5                                                            | 秒，0 表示停用超时，大于 0 表示指定的超时的秒数              | serve-expired-reply-ttl 30                                   |
